...
3. ## Доделаем страницу для создания заказа
    ...
    Так же необходимо создать форму для заполнения адреса доставки (Поле `delivery_address` в модели `Order`).<br>
    При создании заказа нам нужен ещё сам продукт по которому мы будем оформлять заказ,<br>
    но input для него не нужен так как наш адрес уже будет содержать id продукта.<br><br>
